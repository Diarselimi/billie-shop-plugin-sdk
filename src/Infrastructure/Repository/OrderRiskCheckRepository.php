...
class OrderRiskCheckRepository extends AbstractPdoRepository implements OrderRiskCheckRepositoryInterface
{
    ...
    public function findLastFailedRiskChecksByOrderId(int $orderId): CheckResultCollection
    {
        $rows = $this->doFetchAll(
            ...
            ' WHERE ' . self::TABLE_NAME . '.order_id = :order_id' .
            ' AND ' . self::TABLE_NAME . '.is_passed = 0' .
            ' AND ' . self::TABLE_NAME . '.id = (
                SELECT MAX(id) AS id 
                FROM order_risk_checks inner_checks 
                WHERE inner_checks.order_id = :order_id AND inner_checks.risk_check_definition_id = ' . self::TABLE_NAME . '.risk_check_definition_id
            )' .
            ' AND ' . self::TABLE_NAME . '.is_passed = 0',
            ['order_id' => $orderId]
        );
    ...
    {
        $fields = array_map(static function (string $field) {
